...
| # | 能力 | 说明 |
|---|------|------|
| 1 | 启动页依赖检测 | local pty 无外部依赖；tmux-cc 模式检查 tmux binary，不可用时自动 fallback 到 local |
...
└── Backends            ← local pty (默认) / tmux-cc (持久化) / tmux (legacy) / docker / ssh
```
...
| Phase | 内容 | 预估 | 状态 |
|-------|------|------|------|
| **1** | Local PTY 默认 backend：切换 DEFAULT_BACKEND 为 local，LocalPtyAgent 多 pane 支持，deprecate 旧 tmux pipe-pane 路径 | 1 周 | ✅ 已完成 |
| **2** | tmux Control Mode 持久化：ControlModeParser 解析 %output/%begin/%end/%exit，TmuxControlModeRuntime 实现 AgentRuntime，注册 tmux-cc backend + recover 支持 | 2 周 | ✅ 已完成 |
...
| 待定项 | 决策 |
|--------|------|
| **Backend 选择** | 通过 `PMUX_BACKEND` 环境变量或 config.json `backend` 字段指定。有效值：`local`（默认）、`tmux-cc`（持久化）、`tmux`（legacy）。无效值 fallback 到 `local`。 |
